...
public class PremierLeagueDAO {
	...
	public List<Player> getAllVertices(Match match) {
		...
		String sql = "SELECT a.PlayerID, p.Name " + "FROM actions a, players p " + "WHERE MatchID = ? "
	...
	public List<Arco> getArchi(Match match, Map<Integer, Player> playerIdMap) {
		...
		try {
			...
			while (res.next()) {
				...
				Player player1 = playerIdMap.get(res.getInt("a1.PlayerID"));
				Player player2 = playerIdMap.get(res.getInt("a2.PlayerID"));
				
				Double peso = res.getDouble("peso");
							
				Arco a = new Arco(player1, player2, peso);
